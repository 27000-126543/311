...
import {
  ...
  AlertCircle, FileText, Shield, Plus,
} from 'lucide-react'
...
export default function Contract() {
  ...
  const handleSign = async () => {
    if (!data) return
    try {
      const res = await api.post<{ success: boolean; data: ContractData }>(`/contracts/${data.id}/sign`, {
        signature: `${user?.orgName || '签署方'}_${new Date().toISOString()}`,
      })
      const d = (res as any).data
      if (d) setData(d)
      setShowSign(false)
            ...
            {showAmend && (
              <div className="space-y-3 border border-slate-200 rounded-lg p-4 bg-slate-50">
                ...
                <div className="flex gap-2 justify-end">
                  <button onClick={() => { setShowAmend(false); setAmendReason('') }} className="text-sm px-4 py-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-100">取消</button>
                  <button onClick={handleAmend} disabled={!amendReason.trim()} className="text-sm px-4 py-2 rounded-lg bg-[#0F2B46] text-white hover:bg-[#0F2B46]/90 disabled:opacity-50">提交</button>
                </div>
              </div>
            )}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 mt-2 text-xs text-slate-400">
                          <span>{new Date(a.date || a.createdAt || '').toLocaleDateString('zh-CN')}</span>
                          {a.documents && a.documents.length > 0 && <span>{a.documents.length} 个附件</span>}
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setShowSign(false)}>
          <div className="bg-white rounded-xl p-6 w-[480px] space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button onClick={handleSign} className="px-4 py-2 text-sm rounded-lg bg-[#C8A45C] text-white hover:bg-[#C8A45C]/90 flex items-center gap-1">
